...
export class SMLX2ManyField extends X2ManyField {
    ...
    async onAdd({ context, editable } = {}) {
        ...
        const domain = [
            ...
            ["owner_id", "=", this.props.context.default_owner_id],
            ["location_id", "=", this.props.context.default_location_id],
            ["entity_asset_id", "=", this.props.context.default_entity_asset_id]
        ];
        const lot_ids = [];
        this.props.record.data.move_line_ids.records.forEach((line) => {
            if (line.data.lot_id?.[0]) {
                lot_ids.push(line.data.lot_id[0]);
            }
        });
        if (lot_ids.length) {
            domain.push(["lot_id", "not in", lot_ids]);
        }
...
registry.category("fields").add("sq_2_many", smlX2ManyField);
