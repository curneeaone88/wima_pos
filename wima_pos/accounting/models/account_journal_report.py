...
class JournalReportCustomHandler(models.AbstractModel):
    ...
    def _get_custom_display_config(self):
        return {
            ...
            'components': {
                'AccountReportLine': 'wima_pos.JournalReportLine',
            },
            'templates': {
                'AccountReportFilters': 'wima_pos.JournalReportFilters',
                'AccountReportHeader': 'wima_pos.JournalReportHeader',
                'AccountReportLineName': 'wima_pos.JournalReportLineName',
            },
            'pdf_export': {
                'pdf_export_main_table_header': 'wima_pos.journal_report_pdf_export_main_table_header',
                'pdf_export_filters': 'wima_pos.journal_report_pdf_export_filters',
                'pdf_export_main_table_body': 'wima_pos.journal_report_pdf_export_main_table_body',
            },
        }
    ...
    def _query_journal(self, options):
        params = []
        queries = []
        report = self.env.ref('wima_pos.journal_report')
                ...
                if treated_amls_count == 0:
                    ...
                    if self.env.user.has_group('account.group_account_manager'):
                        action = {
                            ...
                            "res_id" : report.id,
                            "views": [[self.env.ref("wima_pos.account_report_form").id, "form"]],
                        }
