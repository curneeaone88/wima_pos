...
class AccountReport(models.Model):
    ...
    def open_report_export_wizard(self, options):
        ...
        view_id = self.env.ref('wima_pos.view_report_export_wizard').id
        ...
        new_wizard = self.with_context(new_context).env['wima_pos.export.wizard'].create({'report_id': self.id})

        return {
            'type': 'ir.actions.act_window',
            'name': _('Export'),
            'view_mode': 'form',
            'res_model': 'wima_pos.export.wizard',
    ...
    def caret_option_open_general_ledger(self, options, params):
        ...
        general_ledger = self.env.ref('wima_pos.general_ledger_report')
        ...
        action_vals = self.env['ir.actions.actions']._for_xml_id('wima_pos.action_account_report_general_ledger')
        action_vals['params'] = {
            'options': gl_options,
            'ignore_session': True,
        }
    ...
    def _generate_common_warnings(self, options, warnings):
        ...
        if options.get('available_tax_units') and options['tax_unit'] == 'company_only':
            warnings['wima_pos.common_warning_tax_unit'] = {}
        ...
        if options.get('date') and options.get('all_entries') is not None:
            ...
            if self.env['account.move'].search_count(period_domain):
                warnings['wima_pos.common_warning_draft_in_period'] = {}
    ...
    def action_display_inactive_sections(self, options):
        ...
        return {
            ...
            'views': [(False, 'list'), (False, 'form')],
            'context': {
                'tree_view_ref': 'wima_pos.account_report_add_sections_tree',
                'active_test': False,
            },
        }
    ...
    def export_to_pdf(self, options):
        ...
        footer = self.env['ir.actions.report']._render_template("wima_pos.internal_layout", values=rcontext)
    ...
    def _get_pdf_export_html(self, options, lines, additional_context=None, template=None):
        ...
        template = custom_print_templates.get('pdf_export_main', 'wima_pos.pdf_export_main')
    ...
    def _compute_is_account_coverage_report_available(self):
        for report in self:
            report.is_account_coverage_report_available = (
                ...
                and
                self.root_report_id in (self.env.ref('wima_pos.profit_and_loss'), self.env.ref('wima_pos.balance_sheet'))
            )
    ...
    def _generate_accounts_coverage_report_xlsx_lines(self):
        ...
        if self.root_report_id == self.env.ref('wima_pos.profit_and_loss'):
