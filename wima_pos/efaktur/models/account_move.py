...
class AccountMove(models.Model):
    ...
    def _constraint_kode_ppn(self):
        ppn_tag = self.env.ref('l10n_id_mceasy.ppn_tag')
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == 'posted'):
            ...
            downpayment_lines = move.line_ids.filtered(lambda x: x.move_id._is_downpayment() and x.display_type == 'product')
            realization_lines = move.line_ids.filtered(lambda x: x.move_id._is_realization() and x.display_type == 'product' and x.is_realization)
            lines = downpayment_lines + realization_lines
            eTax['FG_UANG_MUKA'] = 1 if len(downpayment_lines) > 0 else 2 if len(realization_lines) > 0 else 0
            eTax['UANG_MUKA_DPP'] = int(float_repr(abs(sum(lines.mapped(lambda l: float_round(l.price_subtotal, 0, rounding_method="DOWN")))), 0))
            eTax['UANG_MUKA_PPN'] = int(float_repr(abs(sum(lines.mapped(lambda l: float_round(l.price_total - l.price_subtotal, 0, rounding_method="DOWN")))), 0))

            if eTax['FG_UANG_MUKA'] == 2:
                eTax.update({
                    'JUMLAH_DPP': eTax['UANG_MUKA_DPP'],
                    'JUMLAH_PPN': eTax['UANG_MUKA_PPN'],
                    'UANG_MUKA_DPP': 0,
                    'UANG_MUKA_PPN': 0
                })
            ...
            for line in move.line_ids.filtered(lambda l: l.display_type == 'product'):
                ...
                for tax in line.tax_ids:
                    if tax.amount > 0:
                        tax_line += ((line.faktur_unit_price - line.faktur_discount) * line.quantity) * (tax.amount / 100.0)

                # guarantees price to be tax-excluded
                invoice_line_total_price = line.faktur_unit_price * line.quantity

                line_dict = {
                    'KODE_OBJEK': line.product_id.default_code or '',
                    'NAMA': line.name or '',
                    'HARGA_SATUAN': float_repr(idr.round(line.faktur_unit_price), idr.decimal_places),
                    'JUMLAH_BARANG': line.quantity,
                    'HARGA_TOTAL': idr.round(invoice_line_total_price),
                    'DPP': (line.faktur_unit_price - line.faktur_discount) * line.quantity,
                    'product_id': line.product_id.id,
                }

                if line.faktur_subtotal < 0:
                    for tax in line.tax_ids:
                        free_tax_line += (line.faktur_subtotal * (tax.amount / 100.0)) * -1.0

                    line_dict.update({
                        'DISKON': float_round(invoice_line_total_price - line.faktur_subtotal, idr.decimal_places),
                        'PPN': free_tax_line,
                    })
                    free.append(line_dict)
                elif line.faktur_subtotal != 0.0:
                    invoice_line_discount_m2m = invoice_line_total_price - line.faktur_subtotal

                    line_dict.update({
                        'DISKON': float_round(invoice_line_discount_m2m, idr.decimal_places),
                        'PPN': tax_line,
                    })
            ...
            for sale in sales:
                ...
                sale.update({
                    ...
                    'DISKON': float_repr(sale['DISKON'], idr.decimal_places),
                })
